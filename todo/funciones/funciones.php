...
function Interes(float $capi,float $rent, float $tiempo){
	...
	$y = round((1-(1+$inte)**(-$tiempo))/$inte, 6, PHP_ROUND_HALF_UP);
		if ($x > $y){
			for ($x; $x > $y;){
			$inte -= 0.000001;
			$y = round((1-(1+$inte)**(-$tiempo))/$inte, 4, PHP_ROUND_HALF_UP);
			} 
		} elseif ($x < $y){
			for ($x; $x < $y;){
			$inte += 0.000001;
...
function InteresM(float $mont,float $rent, float $tiempo){
	$x = round(($mont / $rent), 6, PHP_ROUND_HALF_UP);
	$inte = 0.5;
	$y = round(((1+$inte)**($tiempo)-1)/$inte, 6, PHP_ROUND_HALF_UP);
		if ($x > $y){
			for ($x; $x > $y;){
			$inte += 0.000001;
...
echo round(InteresM(20000,2000,9), 6, PHP_ROUND_HALF_UP) * 100;
?>
